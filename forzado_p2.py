...
def punto(t, params):
    w0, gamma, X, w = params
    Fm = X * (np.cos(w*t))
    Theta = (Fm)/(np.sqrt(((w0**2 - w**2)**2)+((gamma*w)**2)))
    return Theta

def punto_2(t, params):
    w0, gamma, X, w = params
    Fm = X * (np.cos(w*t))
    fase = np.arctan((gamma*w)/((w0**2) - (w**2))) #This
    return fase
...
tvac, xvac, vac, Thvac = integrate(euler, sim_params)
...
ax.set(xlabel='time (AU)', ylabel='State (AU)', title='$\omega$ = 3.0')
ax.grid()
